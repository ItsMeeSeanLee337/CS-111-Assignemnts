...
public class ScenarioAnalysis 
{
    ...
    public void computeCO2EmissionsAndCost () 
    {
        ...
        for (int i = 0; i < vehicles.length; i++)
        {
            ...
            {
                //CO2 Emissions
                co2Emission = (vehicles[i].getLease().getMileageAllowance() * (vehicles[i].getLease().getNumberOfMonths() / 12)/ vehicles[i].getFuel().getUsage()) * (Fuel.CO2EMITTED_GASCOMBUSTION);
                vehicles[i].setCO2Emission(co2Emission);
                //Fuel Cost
                fuelCost = (vehicles[i].getLease().getMileageAllowance() * (vehicles[i].getLease().getNumberOfMonths() / 12) * getGasPrice());
            ...
            {
                //CO2 Emissions
                co2Emission =  (vehicles[i].getLease().getMileageAllowance() * (vehicles[i].getLease().getNumberOfMonths() / 12)/ vehicles[i].getFuel().getUsage()) * (vehicles[i].getFuel().getKWhPerCharge()) * (Fuel.CO2EMITTED_GENERATEmWh * 0.001 * 0.45); 
                vehicles[i].setCO2Emission(co2Emission);
                //Fuel Cost
                fuelCost = (vehicles[i].getLease().getMileageAllowance() * (vehicles[i].getLease().getNumberOfMonths() / 12) * getElectricityPrice());
                vehicles[i].setFuelCost(fuelCost);
            }
            //Lease Cost
            leaseCost = vehicles[i].getLease().getDueAtSigning() + (vehicles[i].getLease().getNumberOfMonths() * vehicles[i].getLease().getMonthlyCost());
            //Total Cost
            totalCost = leaseCost + fuelCost + vehicles[i].getOtherCost();
