...
public class WeatherGenerator 
{
    ...
    {
        //longitude is column 0
        //latitude is coumn 1
        int[] returnArray = new int[numberOfDaysInMonth[month - 2]];
        double wetOrDryDay = StdRandom.uniform();
        int row = 0;
        //First Day:
        if (wetOrDryDay <= 0.49)
        {
        ...
        //Subsequent Days:
        for (int i = 1; i < numberOfDaysInMonth[month - 2]; i++)
        {
            if (returnArray[i - 1] == WET)
            {
                wetOrDryDay = StdRandom.uniform();
                //Finding Row:
                for (int j = 0; j < 4001; j++)
                {
                    if ((wetwet[j][0] == longitute) && (wetwet[j][1] == latitude))
                    {
                        row = j;
                        break;
                    }
                }
                if (wetOrDryDay <= wetwet[row][month])
                {
                    returnArray[i] = WET;
                }
                else
            ...
            else
            {
                ...
                for (int j = 0; j < 4001; j++)
                {
                    if ((drywet[j][0] == longitute) && (drywet[j][1] == latitude))
                    {
                        row = j;
                        break;
                    }
                }
                if (wetOrDryDay <= drywet[row][month])
                {
                    returnArray[i] = WET;
                }
                else
    ...
    {
        int longestLengthOfMode = 0;
        int currentLongest = 0;
        for (int i = 0; i < forecast.length; i++)
        {
            if (forecast[i] == mode)
            {
                for (int j = i; j < forecast.length; j++)
                {
                    if (forecast[j] == mode)
                    {
                        currentLongest++;
                ...
                if (currentLongest >= longestLengthOfMode)
                {
                    longestLengthOfMode = currentLongest;
                }
                currentLongest = 0;
            }
        }
        return longestLengthOfMode;
    }
    /*
